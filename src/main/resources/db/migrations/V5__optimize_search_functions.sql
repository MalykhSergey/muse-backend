...
BEGIN
    RETURN QUERY
    -- 1. Поисковый запрос в tsquery
    WITH tsq AS (
        SELECT (websearch_to_tsquery('english', search_query) || websearch_to_tsquery('russian', search_query)) AS q_union
    ),
    -- 2. Предсчёт score, answer_count, tags
    scores AS (
        SELECT post_id,
               SUM(CASE type WHEN 'POSITIVE' THEN 1
                             WHEN 'NEGATIVE' THEN -1 ELSE 0 END) AS score
        FROM votes
        GROUP BY post_id
    ),
    answers AS (
        SELECT parent_id AS post_id, COUNT(*) AS answer_count
        FROM posts
    ...
    -- 3. Подсчёт общего числа найденных постов (без join-ов)
    total AS (
    ...
    -- 4. Выборка только id и rank_val с сортировкой и лимитом
    post_ids AS (
        SELECT p.id,
        ...
        ORDER BY
            ...
            CASE WHEN sort_by = 'SCORE'   AND sort_dir = 'ASC'  THEN 0 END ASC,  -- позже заменим реальным score
