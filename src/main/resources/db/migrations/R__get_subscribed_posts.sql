DROP FUNCTION IF EXISTS get_subscribed_posts;

CREATE OR REPLACE FUNCTION get_subscribed_posts(
...
BEGIN
    ...
    total AS (
        ...
        FROM posts p
        INNER JOIN posts_subscribes ps ON ps.post_id = p.id AND ps.user_id = user_id_param
    ),
    post_ids AS (
        SELECT p.id, ps.is_notification
        FROM posts p
        INNER JOIN posts_subscribes ps ON ps.post_id = p.id AND ps.user_id = user_id_param
